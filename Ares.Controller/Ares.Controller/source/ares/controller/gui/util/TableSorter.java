...
public class TableSorter extends AbstractTableModel {
    ...
    public void saveState(String name, Preferences prefs) {
    	prefs.putInt(name + "SorterCount", sortingColumns.size()); //$NON-NLS-1$
    	for (int i = 0; i < sortingColumns.size(); ++i) {
    		Directive dir = sortingColumns.get(i);
    		prefs.putInt(name + "SorterColumn" + i, dir.column); //$NON-NLS-1$
    		prefs.putInt(name + "SorterDirection" + i, dir.direction); //$NON-NLS-1$
    	}
    }
    
    public void restoreState(String name, Preferences prefs) {
    	sortingColumns.clear();
    	int size = prefs.getInt(name + "SorterCount", 0); //$NON-NLS-1$
    	for (int i = 0; i < size; ++i) {
    		int column = prefs.getInt(name + "SorterColumn" + i, -1); //$NON-NLS-1$
    		int direction = prefs.getInt(name + "SorterDirection" + i, NOT_SORTED); //$NON-NLS-1$
    		if (column != -1) {
    ...
    public interface SortingListener {
    	void sortingStarting();
    	void sortingFinished();
    }
    
    private SortingListener listener;
    
    public void setSortingListener(SortingListener l) { listener = l; }

    private void sortingStatusChanged() {
    	if (listener != null) listener.sortingStarting();
        clearSortingState();
        fireTableDataChanged();
        if (tableHeader != null) {
            tableHeader.repaint();
        }
        if (listener != null) listener.sortingFinished();
    }
    ...
    private class Row implements Comparable<Row> {
        ...
        public boolean equals(Object other) {
          return (other instanceof Row) ? equals((Row)other) : false;
        }
